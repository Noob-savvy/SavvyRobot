...
async def gps(bot, message):
    ...
    try:
        ...
        """
        ---------github :-Ranavanahi-----
        ---------telegram : @ranavanshi_divy-----
        """
        geolocator = Nominatim(user_agent="Divyanshu Rana")
        #         zoom=[0-18]
        ...
        await message.reply_venue(
            latitude, longitude, f"{city}", f"{state} ,{country}", reply_markup=IKM(url)
        )
...
async def distance(bot, message):
    ...
    try:
        ...
        await message.reply_text(
            f"Total distance between {x[0]},{x[1]} and {y[0]},{y[1]} is {distance}"
        )
