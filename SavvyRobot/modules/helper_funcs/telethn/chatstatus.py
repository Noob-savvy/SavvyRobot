...
from SavvyRobot import DRAGONS
from SavvyRobot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def savvy_is_admin(chat_id: int):
    status = False
    savvy = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if savvy.id == user.id:
