...
def sibyl_ban(update: Update, context: CallbackContext) -> Optional[str]:
    ...
    if sibylClient:
        ...
        if data.banned and act in {1, 2}:
            ...
            log_msg += (
                f" • <b>Reason:</b> <code>{data.reason}</code>\n" if data.reason else ""
            )
            log_msg += (
                f" • <b>Ban time:</b> <code>{data.date}</code>" if data.date else ""
            )
    ...
    if sibylClient:
        ...
        for user in users:
            ...
            if data.banned:
                ...
                log_msg += (
                    f" • <b>Ban time:</b> <code>{data.date}</code>\n"
                    if data.date
                    else ""
                )
...
def get_sibyl_info(
    bot: Bot, user: User, detailed: bool = False
    ...
    if data and data.banned:
        ...
        keyboard[0].append(
            InlineKeyboardButton("Appeal", url="https://t.me/SibylRobot")
        )
        reply_markup = InlineKeyboardMarkup(keyboard)
...
if SIBYL_KEY and __name__.split(".")[-1] in ALL_MODULES:
    ...
    dispatcher.add_handler(
        MessageHandler(
            filters=Filters.status_update.new_chat_members, callback=sibyl_ban_alert
        ),
        group=102,
    )
